...
public class PersonController : Controller {
    ...
    [HttpGet]
    public ViewResult Index(SortByType? sortByDesc, GenderType? gender) {
        GenderDropDown();
        ...
        //sort people by age
        if (sortByDesc != null) {
            if (sortByDesc == SortByType.Ascending)
                peopleList = peopleList.OrderByDescending(person => person.DateOfBirth);
            else if (sortByDesc == SortByType.Descending)
    ...
    public ViewResult DeletePerson(Guid id) {
        try {
            ...
            if (person == null) throw new Exception("Person was not found");
